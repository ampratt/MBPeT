...
public class ConfirmDeleteTRTWindow extends Window {
	...
	JPAContainer<Parameters> parameterscontainer;
	JPAContainer<TRT> trtcontainer;
	Parameters parentparameters;
	
	public ConfirmDeleteTRTWindow(TRT currtrt, Parameters currParams, String message) {
        super("Heads Up!");
        center();
        setResizable(false);
        setClosable(false);
...
//            submit.setClickShortcut(KeyCode.ENTER);
            delete.addClickListener(new ClickListener() {
				public void buttonClick(ClickEvent event) {
	        		...
	        		// for notification
	        		String deleteditem = currTrt.getAction();
	        		
	                                            
	                // 1. remove child from Parameter's list
//	        		parentparameters = parameterscontainer.getItem(currTrt.getParentparameter().getId()).getEntity();
	                parentparameters.removeTRT(currTrt);
	                parameterscontainer.addEntity(parentparameters);
	                
	                // 3. delete model from container
	                trtcontainer.removeItem(currTrt.getId());
